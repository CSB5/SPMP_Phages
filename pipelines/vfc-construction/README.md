...
This is a bioinformatics pipeline for constructing viral clusters (VCs) via hierarchical clustering of a set of input vOTUs and reference vOTUs with known taxonomic assignments. It estimates viral genome completeness using CheckV, defines protein clusters (PCs), and performs UPGMA clustering on high-quality and complete vOTUs based on % shared distinct PCs. Inspection of reference vOTU clustering at different distance thresholds led to viral family-level clusters (VFCs) being defined at 20% shared PCs. Medium-quality vOTUs are subsequently recruited into the VFCs if they share 30% PCs with members of that VFC (this threshold was chosen to minimize ambiguous assignments to >1 VFC).
...
```bash
conda create -n vfc-construction -c conda-forge -c bioconda snakemake=5.26.0 checkv=1.0.1 genomad=1.8.1 seqtk=1.4 diamond=2.1.8 mcl=22.282 pandas=2.2.0 numpy=1.26.3 scipy=1.12.0
conda activate vfc-construction
```
...
* **geNomad database:**
  See [https://portal.nersc.gov/genomad/quickstart.html#downloading-the-database](https://portal.nersc.gov/genomad/quickstart.html#downloading-the-database). Save `genomad_db/`, or a symlink to it, in the `resources/` directory.
...
* **`vr95_c50-upgma.tsv`**
  A tab-separated table containing Viral Cluster (VC) labels for all >=90% complete, and classified 50-90% complete, input and reference vOTUs across a range of distance thresholds.

> **Interpretation:** Columns like `VC60` represent viral clusters formed at a 60% distance threshold (i.e., 40% shared distinct PCs). These hierarchical clusters enable comparison with reference clade assignments, facilitating analysis of viral groups at the genus, family, and order levels.
...
> Chen, H. et al. [...] (2026).
See Supplementary Fig. 4 for a schematic overview of this pipeline.
